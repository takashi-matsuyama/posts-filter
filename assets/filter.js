...
(function($){
  ...
  function filter_term(taxonomy_type, this_elm, option) {
    ...
    if (this_elm.hasClass(active_select_term) === false) {
      ...
      $('#'+ select_taxonomy +'-'+ taxonomy_type + option).find('.'+ select_term_item).removeClass(active_select_term);
      $('.list-terms-' + taxonomy_type).find('.'+ list_term_item + option).removeClass(active_list_term);
      //基本の切り替え
      this_elm.toggleClass(active_select_term);
      this_elm.closest(select_taxonomy_elm).addClass(has_active);
      $('[data-'+ filter_data_list + option +'="'+ data_value +'"]').toggleClass(active_list_term);
    } else {
      ...
      $('[data-'+ filter_data_list + option +'="'+ data_value +'"]').removeClass(active_list_term);
    }
    //選択されたメニューの数をクリック毎に更新してdata属性に挿入
    var active_select_term_count = nav_taxonomy_elm.find('.'+ active_select_term).length;
    ...
    list_filter_elm.each( function() {
      ...
      if ( $(this).attr('data-match-list_term') == nav_taxonomy_elm.attr('data-match-select_term') ) {
        //一致するつリスト
        $(this).fadeIn();
  ...
  function select_taxonomy_standard(taxonomy_type) {
    $('#'+ select_taxonomy +'-'+ taxonomy_type).find('.select-toggle-'+ taxonomy_type).on('change', function () {
  ...
  function reset_select_taxonomy(taxonomy_type, this_elm) {
    var select_taxonomy_this = this_elm.closest('#'+ select_taxonomy +'-'+ taxonomy_type);
  ...
  select_reset_elm.on('change', '.'+ select_toggle, function() {
    var taxonomy_type = $(this).closest(select_taxonomy_elm).data(select_taxonomy);
    var this_elm = $(this).closest(select_taxonomy_elm).find('.'+ active_select_term);
    var option = '';
    filter_term(taxonomy_type, this_elm, option); // 実行本体：ターム切り替えとカスタム投稿のフィルタ表示
    if( $(this).closest(select_taxonomy_elm).find(select_term_item_elm).hasClass(active_select_term) === true ) {
    ...
    $(this).closest(select_taxonomy_elm).find('input[type="checkbox"]').prop('checked', false);
  });
  ...
  accordion_trigger_elm.on('click', 'a', function(e) {
    ...
    $(this).closest(accordion_trigger_elm).next(accordion_contents_elm).fadeToggle('fast');
  });



  /*** チェックボックスを択一に変更 ***/
  $('.'+ select_toggle).children('input[type="checkbox"]').on('change', function() {
  ...
  ******/
  ...
  function more_trigger_toggle() {
    ...
    var found_posts_count_val = $('#'+ wp_query_count).children('.'+ found_posts_count).text();
  ...
  function active_list_term_election(event){
    ...
    if( event !== 'load' ) {
      $('.'+ active_select_term).each( function() {
        ...
        $('[data-list-term="'+ active_data_select_term +'"]').addClass(active_list_term);
      });
    }
    var active_data_article_term = article_filter_elm.attr('data-'+ article_filter_data_term);
    $('[data-list-term="'+ active_data_article_term +'"]').addClass(active_list_term);
  };
  ...
  function ccc_my_favorite_initial_ajax() {
    ...
    if( typeof CCC_MY_FAVORITE_UPDATE !== 'undefined' && typeof CCC_MY_FAVORITE_GET !== 'undefined' ) {
  ...
  $(document).on('change', '.'+ select_toggle, function() {
    ...
    nav_taxonomy_elm.find(select_taxonomy_elm).each( function() {
      ...
      var data_standard = $(this).data(select_taxonomy);
      if( data_standard ) {
        var data_standard_val = $("[name='"+ data_standard +"[]']:checked").val();
  ...
  article_filter_elm.find('.nav-taxonomy-toggle-button').on('click', function(e) {
    e.preventDefault();
